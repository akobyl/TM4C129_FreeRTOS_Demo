                                   ...
                                   const UBaseType_t uxItemSize, const uint8_t ucQueueType )
{
    ...
    else
    {
        ...
        xQueueSizeInBytes = ( size_t ) ( uxQueueLength * uxItemSize ) +
                            ( size_t )
                            1; /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
    }

    /* Allocate the new queue structure and storage area. */
    pcAllocatedBuffer = ( int8_t *) pvPortMalloc( sizeof( Queue_t ) +
                        xQueueSizeInBytes );

    if ( pcAllocatedBuffer != NULL )
    {
        pxNewQueue = ( Queue_t *)
                     pcAllocatedBuffer;  /*lint !e826 MISRA The buffer cannot be too small because it was dimensioned by sizeof( Queue_t ) + xQueueSizeInBytes. */
...
BaseType_t xQueueIsQueueFullFromISR( const QueueHandle_t xQueue )
{
    ...
    if ( ( ( Queue_t *) xQueue )->uxMessagesWaiting == ( ( Queue_t *)
            xQueue )->uxLength )
    {
        xReturn = pdTRUE;
    }
        ...
        TickType_t const xTicksToWait )
{
    ...
    ( void ) xQueueGenericReceive( ( QueueHandle_t ) xQueueSet, &xReturn,
                                   xTicksToWait,
